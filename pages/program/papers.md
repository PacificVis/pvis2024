---
layout: single
title: 'Papers'
permalink: '/program/papers/'
date: 2023-02-10
---

---
# Apr 24 (Wed), 13:30-15:33
*AI and Visual Analytics*\
*Session Chair: To be announce*

**VIOLET: Visual Analytics for Explainable Quantum Neural Networks**\
Shaolun Ruan, Zhiding Liang, Qiang Guan, Paul Griffin, Xiaolin Wen, Yanna Lin, Yong Wang
                        
**Reinforced Labels: Multi-Agent Deep Reinforcement Learning for Point-Feature Label Placement**\
Petr Bobák, Ladislav Čmolík, Martin Čadík
                        
**Visual Analytics for Efficient Image Exploration and User-Guided Image Captioning**\
Yiran Li, Junpeng Wang, Prince O. Aboagye, Chin-Chia Michael Yeh, Yan Zheng, Liang Wang, Wei Zhang, Kwan-Liu Ma
                        
**PM-Vis: A Visual Analytics System for Tracing and Analyzing the Evolution of Pottery Motifs**\
Li Jinchen, Chufan Lai, Hai Zhang, Xiaoru Yuan
                        
**ColorNetVis: An Interactive Color Network Analysis System for Exploring the Color Composition of Traditional Chinese Painting**\
Xiaojiao Chen, Qinghua Liu, Yonghao Chen, Ruihan Wang, Yang You, Wanxin Deng, Wei Chen, Xiaosong  Wang
                        
**Exploring Visualization for Fairness in AI Education**\
Xinyuan Yan, Youjia Zhou,  Arul Mishra, Himanshu Mishra, Bei Wang
                        
**Graph-Neural-Network-Based User Intent Understanding for Visual Analytics**\
Yue Wang, Yusheng Qi, Guozheng Li, Xiaolong (Luke) Zhang, Siming Chen
                        
**Trust Junk and Evil Knobs: Calibrating Trust in AI Visualization**\
Emily Wall, Laura Matzen, Mennatallah El-Assady, Peta Masters, Helia Hosseinpour, Alex Endert, Rita Borgo, Duen Horng Chau, Adam Perer, Harald Schupp, Hendrik Strobelt, Lace M. Padilla
                        
**Data Movement Visualized - A Unified Performance Analysis Framework to Track Data Movement in Heterogeneous Architectures**\
Suraj Padmanaban Kesavan, Harsh Bhatia, Keshav Dasu, Olga Pearce, Kwan-Liu Ma
                        
---
# Apr 24 (Wed), 16:00-17:51
*Scientific and Geographic Visualization*\
*Session Chair: To be announce*

**Animating Hypothetical Trips to Communicate Space-Based Temporal Uncertainty on Digital Maps**\
Morgane Koval, Yvonne Jansen, Fanny Chevalier
                        
**Web-Based Flow Visualization in Quotient Space and S³**\
Christian Lang, Egzon Miftari, Peter Albers, Filip Sadlo
                        
**Interactive Visualization of Time-Varying Flow Fields Using Particle Tracing Neural Networks**\
Mengjiao Han, Jixian Li, Sudhanshu Sane, Shubham Gupta, Bei Wang, Steve Petruzza, Chris R. Johnson
                        
**Towards Differentiable Electron Microscopy Simulation: Methods and Applications**\
Ngan Nguyen, Feng Liang, Dominik Engel, Ciril Bohak, Ondřej Strnad, Timo Ropinski, Ivan Viola
                        
**ECNR: Efficient Compressive Neural Representation of Time-Varying Volumetric Datasets**\
Kaiyuan Tang, Chaoli Wang
                        
**Dynamic-Scene-Graph-Supported Visual Understanding of Autonomous Driving Scenario**\
Ting Liu, Dong Sun, Chongke Bi, Yi Sun, Siming Chen
                        
**Interpreting Autonomous Driving Corner Cases: A Visual Analytics Approach**\
Yi Sun, Zekai Shao, Xingyu Qiu, Yun Li, Ting Liu, Linbing Xiang, Dong Sun, Siming Chen
                        
**EmoGeoCity: Interactive Visual Exploration of City’s Historical and Cultural Evolution Based on Emotional Geography**\
Yutong Yang, Yinuo Liu, Qishuo Bai, Tianyi Zhou, Ziduo Ye, Xiaoju Dong
                        
**ViSCitR: Visual Summarization and Comparison of Hotel Reviews**\
Franziska Huth, Fabian Beck, Johannes Knittel, Shahid Latif, Steffen Koch, Thomas Ertl
                        
---
# Apr 25 (Thu), 09:00-11:03
*Graph and Network*\
*Session Chair: To be announce*

**Sticky Links: Encoding Quantitative Data of Graph Edges**\
Min Lu, Xiangfang Zeng, Joel Lanir, Xiaoqin Sun, Guozheng Li, Daniel CohenOr, Hui Huang
                        
**The Multi-Dimensional Landscape of Graph Drawing Metrics**\
Gavin J. Mooney, Helen C. Purchase, Michael Wybrow, Stephen Kobourov
                        
**Treebar Maps: Schematic Representation of Networks at Scale**\
Giuseppe Di Battista, Fabrizio Grosso, Silvia Montorselli, Maurizio Patrignani
                        
**GraphFederator: Federated Visual Analysis for Multi-party Graphs**\
Dongming Han, Wei Chen, Rusheng Pan, Yijing Liu, Jiehui Zhou, Haozhe Feng, Tianye Zhang, Haiyang Zhu, Xumeng Wang, Jianrong Tao, Changjie Fan, Minfeng Zhu, Xiaolong (Luke) Zhang
                        
**Area-adaptive Drawing of Rooted Trees**\
Kazuo Misue
                        
**KG-PRE-view: Democratizing A TVCG Knowledge Graph through Visual Explorations**\
Yamei Tu, Rui Qiu, Han-Wei Shen
                        
**An Authoring Tool for Graph visualizations**\
Dongming Han, Wei Chen, Jiacheng Pan, Ran Chen, Haiyang Zhu, Xumeng Wang, Zhen Wen, Luoxuan Weng, Minfeng Zhu, Yingcai Wu, Rüdiger Westermann
                        
**Contextual Matrix Orderings for Graph Collections**\
Nathan van Beusekom, Wouter Meulemans, Bettina Speckmann
                        
**Cluster-Faithful Graph Visualization: Metrics and Algorithms**\
Shijun Cai, Seok-Hee Hong, Amyra Meidiana, Peter Eades, Daniel Keim
                        
**Improved Lightweight Rendering of Road Networks based on Contraction Hierarchies**\
Lukas Berner, Johannes Erwerle, Stefan Funke, Claudius Proissl, Florian Rieg, Sabine Storandt
                        
---
# Apr 25 (Thu), 13:30-15:33
*Representation and Framework*\
*Session Chair: To be announce*

**JsonCurer: Data Quality Management for JSON Based on an Aggregated Schema**\
Kai Xiong, Xinyi Xu, Siwei Fu, Di Weng, Yongheng Wang, Yingcai Wu
                        
**Visual Analytics for Efficient Image Exploration and User-Guided Image Captioning**\
Yiran Li, Junpeng Wang, Prince O. Aboagye, Chin-Chia Michael Yeh, Yan Zheng, Liang Wang, Wei Zhang, Kwan-Liu Ma
                        
**Adaptive Sampling of 3D Spatial Correlations for Focus+Context  Visualization**\
Christoph Neuhauser, Josef Stumpfegger, Rüdiger Westermann
                        
**Comparative Evaluation of Animated Scatter Plot Transitions**\
Nils Rodrigues, Frederik L. Dennig, Vincent Brandt, Daniel Keim, Daniel Weiskopf
                        
**CoInsight: Visual Storytelling for Hierarchical Tables with Connected Insights**\
Guozheng Li, Runfei Li, Yunshan Feng, Yu Zhang, Yuyu Luo, Chi Harold Liu
                        
**Understanding and Automating Graphical Annotations on Animated Scatterplots**\
Danqing Shi, Antti Oulasvirta, Tino Weinkauf, Nan Cao
                        
**ENS-t-SNE: Embedding Neighborhoods Simultaneously t-SNE**\
Jacob Miller, Vahan Huroyan, Raymundo Navarrete, Md Iqbal Hossain, Stephen Kobourov
                        
**Extending Adjacency Matrices to 3D with Triangles**\
Rusheng Pan, Helen C. Purchase, Tim Dwyer, Wei Chen
                        
**Sample2SQL: A Visual Interface for Querying Risky Enterprises**\
Hongjia Wu, ShanChen Zou, Jiazhi Xia, HongXin Zhang, Wei Chen
                        
**Efficient Level-Crossing Probability Calculation for Gaussian Process Modeled Data**\
Haoyu Li, Isaac J Michaud, Ayan Biswas, Han-Wei Shen
                        
---
# Apr 25 (Thu), 16:00-17:57
*Application and Interpretation*\
*Session Chair: To be announce*

**ScrollTimes: Tracing the Provenance of Paintings as a Window into History**\
Wei Zhang, Kam Kwai Wong, Yitian Chen, Ailing Jia, Luwei Wang, Jian-Wei Zhang, Lechao Cheng, Huamin Qu, Wei Chen
                        
**TacPrint: Visualizing the Biomechanical Fingerprint in Table Tennis**\
Jiachen Wang, Ji Ma, Zheng Zhou, Xiao Xie, Hui Zhang, Yingcai Wu, Huamin Qu
                        
**A Comprehensive Evaluation Framework of Software Visualization Effectiveness**\
Hakam Alomari,Hakam W. Alomari, Christopher Vendome, Lane Rizkallah
                        
**Same Data, Diverging Perspectives: the Power of Visualizations to Elicit Competing Interpretations**\
Cindy Xiong Bearfield, Lisanne van Weelden, Adam Waytz, Steven L. Franconeri
                        
**Enhancing Single-Frame Supervision for Better Temporal Action Localization**\
Changjian Chen, Jiashu Chen, Weikai Yang, Haoze Wang, Johannes Knittel, Xibin Zhao, Steffen Koch, Thomas Ertl, Shixia Liu
                        
**Towards Creating Comprehensible Visualizations**\
Maryam Rezaie, Melanie Tory, Sheelagh Carpendale
                        
**Exploring Mid-Air Hand Interaction in Data Visualization**\
Zona Kostic, Catherine Dumas, Sarah Pratt, Johanna Beyer
                        
**VisCollage: Annotative Collages for Organizing Data Event Charts**\
Xiao Han Li, Yi-Ting Hung, Jia-Yu Pan, Wen-Chieh Lin
                        
---
# Apr 26 (Fri), 09:00-10:00
*Visualization Technique, Graph and Time-series Visualization*\
*Session Chair: To be announce*

**Towards Creating Comprehensible Visualizations**\
Maryam Rezaie, Melanie Tory, Sheelagh Carpendale
                        
**$\beta$-skeleton Shape-based Metrics for Large and Complex Graph Drawings**\
Seok-Hee Hong, Patrick Eades
                        
**Cell Size Optimization for Graph Drawing on Torus**\
Misato Watanabe, Yosuke Onoue
                        
**A-Map: Interactive Visual Exploration of Intercity Accessibility Dynamics Based on Railway Network Data**\
Kaichen Nie, Hanning Shao, Yuchu Luo, Min Tian, Hao Wu, Wei Zeng, Xin Fu, Xiaoru Yuan
                        
**Towards Understanding the Authoring Strategy and Effectiveness of Visualization Sketches**\
Ruike Jiang, Yiheng Liang, Hanning Shao, Le Liu, Xiaoru Yuan
                        
**TIMESERIESMAKER: Interactive Time Series Composition in No Time**\
Franziska Becker, Tanja Blascheck
                        
---
# Apr 26 (Fri), 10:00-11:00
*AI+VIS and Visual Analytics Application*\
*Session Chair: To be announce*

**A Systematic Evaluation of Visual GenAI Model Outputs**\
Tica Lin, Hanspeter Pfister, Jui-Hsien Wang
                        
**PromptLandscape: Guiding Prompts Exploration and Analysis with Visualization**\
Junpeng Wang, Chin-Chia Michael Yeh, Yujie Fan, Xin Dai, Yan Zheng, Liang Wang, Wei Zhang
                        
**Evaluating how interactive visualizations can assist in finding samples where and how computer vision models make mistakes**\
Hayeong Song, Gonzalo Ramos, Peter Bodik
                        
**IoLens: Visual Analytics System for Exploring Storage I/O Tracking Process**\
Changmin Jeon, Jiwon Ha, Hyolim Hong, Hyeon Jeon, Hyeonsang Eom, Heonyoung Yeom, Jinwook Seo
                        
**SG-RAD: A Visual Analytics Approach in Subgroup and Risk Factors Discovery and Analysis**\
Nathania Josephine, Yi-Ju Jean Lee, Pei-Chen Chang, Hsiang-Han Chen, Ko-Chih Wang
                        
**MaugVLink: Augmenting Mathematical Formulas with Visual Links**\
Min-Jui Lee, Bing-Yu Chen
                        
